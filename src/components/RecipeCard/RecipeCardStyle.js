...
import { breakpoints, colors, border } from "../../lib/style/theme";
...
export const RecipieCardWrapper = styled.div`
  ...
  background: ${({ theme }) =>
    theme.mode === "dark" ? theme.bgPrimaryLight900 : theme.primaryLight};
  box-shadow: rgba(0, 0, 0, 0.1) 0px 3px 8px;
  border: 1px solid
    ${({ theme }) =>
      theme.mode === "dark" ? theme.bgPrimaryLight700 : theme.tableBorder};
  ...
  @media (hover: hover) and (pointer: fine) {
    :hover {
      ...
      box-shadow: ${({ theme }) =>
        theme.mode === "dark"
          ? `${theme.primary700} 0px 6px 15px`
          : `${theme.primary200} 0px 6px 15px`};
    }
  }
`;
...
export const RecipeCategory = styled.p`
  ...
  color: ${({ theme }) =>
    theme.mode === "dark" ? theme.white : theme.textSecondary};
`;
